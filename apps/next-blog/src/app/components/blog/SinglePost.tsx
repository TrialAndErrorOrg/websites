...
import { getTweet } from '../core/Tweet/getTweetData'
...
console.dir(JSON.stringify(bod), { depth: null })
...
const bodySplitByEmbeddedTweet = await Promise.all(
  bodySplitByEmbeddedTweetFigure.map(async (body) => {
    const tweet = body.match(/<oembed url="(https?:\/\/twitter\.com\/.*?\/status\/\d+)"><\/oembed>/)
    console.log({ tweet })
...
console.dir(bodySplitByEmbeddedTweet, { depth: null })
// const proseAnchorSleekUnderline = sleekUnderline.map((c) => `prose-a:${c}`).join(' ')

return <section className="relative mx-auto flex flex-col bg-white dark:bg-slate-700">
  {
  ...
  <div className="relative w-full border-t-2 border-black bg-white">
    <article
      className="relative mx-auto grid w-full max-w-[100rem] grid-cols-12 border-black bg-white py-10 dark:border-white dark:bg-slate-700"
    >
      ...
      <header className="col-span-10 col-start-2 my-1 md:col-span-8 md:col-start-3 md:my-2">
        <h1
          className="leading-tighter font-heading max-w-3xl text-3xl font-bold tracking-tighter text-black dark:text-white md:text-6xl"
        >
          {post.title}
        </h1>
        {
          post.doi && (
            <a className="sleek-underline text-sm font-semibold" href={`https://doi.org/${post.doi}`}>
      ...
      <div
        className="container col-span-10 col-start-2 mx-auto my-4 max-w-3xl md:col-span-5 md:col-start-3 md:mt-8"
      >
      ...
      <div
        className="col-span-11 col-start-2 my-8 ml-4 flex h-4 items-center justify-start gap-2 p-1 md:sticky md:top-40 md:col-span-1 md:col-start-1 md:flex-col md:gap-4"
      >
        <a
...

      {
        tableOfContents.length > 0 && (
          <div className="sticky top-[3.8125rem] right-[max(0px,calc(50%-45rem))] z-20 col-span-2 col-start-11 hidden h-max overflow-y-auto py-10  lg:block lg:w-64 xl:w-[19.5rem]">
      ...
      <div className="col-span-10 col-start-2 grid gap-3 md:col-span-5 md:col-start-3">
        {authors.map((author) => <Author className="" author={author} />)}
        <div className="mt-16">
          <License post={post} />
        </div>
      </div>
    ...
    <div className="relative mx-auto grid max-w-[100rem] grid-cols-12 bg-white py-8 dark:bg-slate-700">
      <div
        className={`md:col-span-7 col-span-10 col-start-2 md:col-start-3 grid ${
    ...
    <div
      className="relative mx-auto grid max-w-[100rem] grid-cols-12 bg-white py-8 dark:bg-slate-700 dark:text-gray-50"
    >
      ...
      <div
        className="col-span-10 col-start-2 flex max-w-5xl flex-col gap-4 md:col-span-9 md:col-start-3"
      >
        {
