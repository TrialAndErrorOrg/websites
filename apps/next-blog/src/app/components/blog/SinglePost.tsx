...
import { type Section, TableOfContents } from "../client/TableOfContents";
...
export async function SinglePost(props: Props) {
	...
	const getHeadings = (body: string) =>
		(body?.match(headingRegex)?.map((heading) => {
			const [, tag, number, properties, content] =
				heading.match(headingRegex) ?? [];
			return {
				title: content,
				children: [] as Section[],
	...
	const getTableOfContents = (body: string) => {
		...
		h2s.forEach((h2, i) => {
			...
			const textBetweenH2s = body.slice(
				body.indexOf(h2) + h2.length,
				nextH2Index,
			);
			sections.push({
				title: h2Text,
				...
				<article className="relative mx-auto grid w-full max-w-[100rem] grid-cols-12 border-black bg-white py-10 dark:border-white dark:bg-slate-700">
					<p className="col-span-8 col-start-2 max-w-3xl md:col-start-3">
						<time dateTime={(post.publishDate ?? post.publishedAt)?.toString()}>
							{getFormattedDate(
								(post.publishDate ?? post.publishedAt)?.toString(),
							)}
						</time>{" "}
					...
					{post.doi && (
						<p className="col-span-8 col-start-2 md:col-start-3">
							DOI:{" "}
							<a
								className="sleek-underline font-semibold"
						...
						{authors.map((author, idx) => {
							...
							return (
								...
								>
									...
									{author.orcid && (
										<a
											className="button-sleek !p-0 !text-[#a6ce39]"
											href={author.orcid}
										>
											<FaOrcid />
