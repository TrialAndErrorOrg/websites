...
const getHighlightedParts = (result: string, query?: string) => {
	...
	const parts = thing
		.split(/\/ais-highlight__|__ais-highlight/)
		?.map((part) => {
			if (/__.*?__/.test(part)) {
				return {
					isHighlighted: true,
...
export function SearchModal({
	...
	const results =
		res as unknown as MeiliSearchResponse<MeiliSearchBlogPostResult>;
			...
			>
				...
				>
					<Combobox
						...
						onChange={(item: MeiliSearchBlogPostResult) =>
																...
																<>
																	...
																	>
																		<Image
																			...
																			alt=""
																			height={
																				item.image?.formats?.thumbnail.height ??
																				item.image?.height
																			}
																			width={
																				item.image?.formats?.thumbnail.width ??
																				item.image?.width
																			}
																	...
																	<div className="ml-4 flex-auto">
																		...
																		>
																			{item._highlightResult.blog_tags.map(
																				(tag) => (
																					<span
																						key={tag.slug.value}
										...
										{activeOption && (
											<div className="hidden h-96 w-1/2 flex-none flex-col divide-y-2 divide-black overflow-y-auto dark:divide-slate-600 sm:flex">
												<div className="flex-none p-6 text-center">
													<img
														src={
															activeOption.image?.formats?.thumbnail.url ??
															activeOption.image?.url
														}
														alt=""
														height={
													...
													<h2 className="mt-3 font-semibold text-black dark:text-slate-50">
														{getHighlightedParts(
															activeOption._highlightResult.title.value,
														).map((part, idx) => {
															return part.isHighlighted ? (
																<mark
																	key={part.value + idx}
													...
													<p className="text-sm leading-6 text-black dark:text-slate-400">
														{getHighlightedParts(
															activeOption._highlightResult.excerpt.value,
														).map((part, idx) => {
															return part.isHighlighted ? (
																<mark
																	key={part.value + idx}
												...
												<div className="flex flex-auto flex-col justify-between p-6">
													...
													>
														{getHighlightedParts(
															cleanHTML(
																activeOption._highlightResult.body.value,
															),
															q,
														).map((part, idx) =>
															part.isHighlighted ? (
