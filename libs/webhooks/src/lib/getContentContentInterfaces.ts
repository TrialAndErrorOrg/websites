...
import { ContentContentInterface, ContentContentInterfaceInterface } from '../src/types'
import tryCatch from './utils/try-catch'

export async function getContentContentInterfaces() {
  const [contentContentInterfaceInterfacesRes, collectionError]: [any, any] = await tryCatch(
    fetch(`http://localhost:1337/api/${process.env.STRAPI_WEBFLOW_TYPES_COLLECTION_NAME}`, {
      headers: {
